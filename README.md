...
GitHub Actions：`.github/workflows/weekly-autopublish.yml`（北京时间每周一 08:55 自动采集上一周一到周日的信息，按七类生成有内容的板块、提交并触发 Vercel）。首次使用请在仓库 **Settings → Actions → General** 将 **Workflow permissions** 设为 **Read and write permissions**（详见部署文档第 5 节）。
...
- 仅更新目标周摘录并 build：`npm run weekly:digest`
