...
公众号/服务号采集：在 `config/weekly-rss.json` 的 `wechatFeeds` 增加可访问 RSS 地址，或配置 `rssHubBase` 后填写 `biz`、`hid`、可选 `cid`。采集仍只写入标题、日期、原文链接；正文按七个行业维度归类，重点覆盖 K12 政策、教辅政策、出版社/教辅公司数智化、局社采购合作、科技合作、行业评论和政策解读；没有本周可靠信息就不写。
