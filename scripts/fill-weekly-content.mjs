...
function buildPrompt({ weekCode, dateCode, digest, example, prevExcerpt }) {
  return `你是一位教辅行业与 K12 教育政策分析师。请撰写 **${weekCode}** 周报的 Markdown 正文（仅第一至第十章，不要写第十一章）。

## 输出要求
1. 语言：简体中文。
2. 只输出从 \`## 一、本周核心摘要（3-5条）\` 到 \`## 十、出版社/教辅公司：跨行及与科技公司深度合作\` 的全部内容，不要输出一级标题和「更新时间」行。
3. 结构、层级、字段名必须与「范例周报」一致（事件/来源/要点/影响判断等子项保留）。
4. 事实须基于下方「本周 RSS 摘录」；可结合常识做行业解读，但 **不得编造** 不存在的文件号、日期、机构名称；无可靠来源时写「本周公开稿未见」或「延续上周跟踪」。
5. 第一章写 **3–5 条** 有信息量的摘要，关键数字、日期、地名用 **加粗**；**每条摘要末尾**须附「 · [原文](url)」（url 须来自 RSS 摘录或权威官网，勿虚构）。
6. 「来源」行、政策条目的 **原文链接** 字段须使用 Markdown 链接「[原文](url)」或「[文件标题](url)」，确保二至十章每条新闻/政策均可点击跳转。
7. 语气与篇幅参照范例，每周内容须与 RSS 及上周摘要区分，避免照抄范例。
